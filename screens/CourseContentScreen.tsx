...
import {
  getAllAssignmentOfCourseApi,
  getAllChapterOfCourseApi,
  getAllFaqOfCourseApi,
} from "../api";
...
function InfoCard(props: InfoCardProps) {
  ...
  useFocusEffect(() => {
    ...
    if (false && (faqData.cid < 0 || faqData.cid != Number(props.cid))) {
      loadFaqData({ cid: Number(props.cid) });
    }
  });
  ...
  // 加载常见问题
  const loadFaqData = async (props: loadFqaDataProps): Promise<any> => {
    if (props.cid == faqData.cid) {
      return false;
    }
    if (Number(props.cid) < 0) {
      return false;
    ...
    console.log("CCS-157- 訪問Faq API", props.cid);
    const getFaqResult = await getAllFaqOfCourseApi<any>({
      cid: props.cid,
    });

    try {
      if (getFaqResult) {
        if (getFaqResult["code"] == 0) {
          console.log(
            "CourseChapterScreen 381 加载常见问题数据正常",
            getFaqResult
          );
          const data = getFaqResult["data"];
          const _faqData: FaqDataType = {
            cid: props.cid,
            dataUrl: data,
          };
          setFaqData(_faqData);
          console.log("CCS-FAQ data 99:", _faqData, faqData);
          return faqData;
        } else if (
          getFaqResult["code"] == 1 &&
          getFaqResult["message"] == "资源不存在"
        ) {
          setFaqData({
            cid: props.cid,
            dataUrl: "",
        ...
        console.log("CCS-170: load FAQ no result!");
        // setLoading(false);
        return false;
      }
    } catch (err) {
      console.log("CourseChapterScreen FAQ 302 ERR", err);
      // setLoading(false);
      return false;
    }
  };
...
function RenderChapterItem(props: RenderChapterItemProps) {
  ...
  const [isAssignmentOpen, setAssignmentOpen] = useState<number>(-1);
  ...
  // 作业信息使用的下列动画
  const assignmentToggleOpen = (assignmentIndex: number) => {
    if (assignmentIndex != isAssignmentOpen) {
      setAssignmentOpen(assignmentIndex);
    } else {
      setAssignmentOpen(-1);
    }
    LayoutAnimation.easeInEaseOut();
    ...
    <>
      ...
      <TabView value={tabIndex} onChange={setTabIndex} animationType="spring">
                ...
                {assignmentDatas.map((data, index) => (
                  <>
                    <ListItem.Accordion
                      ...
                      isExpanded={index == isAssignmentOpen}
                      icon={<Icon name="place" />}
                      // leftRotate
                      onPress={() => {
                        assignmentToggleOpen(index);
                      }}
                    >
                      <ListItem
                        key={index}
                      ...
                      <ListItem>
                        ...
                        >
                          <Button
                            ...
                            disabledTitleStyle={{ margin: 0, color: "#2089DC" }}
                          ...
                          <Button
                            ...
                            title={"下载作业"}
                            onPress={() => openUrl(data.uploadedFile)}
                            buttonStyle={{ borderRadius: 10 }}
                            containerStyle={{ margin: 5 }}
                            disabledStyle={{
                              borderWidth: 2,
        ...
        <TabView.Item style={{ width: "100%" }}>
          {/* 常见问题 */}
          <ListItem>
              <ListItem.Title>此课程还没有发布常见问题!</ListItem.Title>
          </ListItem>
          {/* <ListItem>
            <ListItem.Content>
              <ListItem.Title style={{ color: "black" }}>
                {!faqData.dataUrl || faqData.dataUrl == ""
                  ? "该课程没有发布常问题"
                  : "常见问题"}
              </ListItem.Title>
            </ListItem.Content>
          ...
          {faqData.dataUrl == "" && (
            <Button
              disabled={!faqData.dataUrl || faqData.dataUrl == ""}
              title={"查看常见问题"}
              onPress={() => openUrl(faqData.dataUrl)}
              buttonStyle={{ borderRadius: 10 }}
              containerStyle={{ margin: 5 }}
              disabledStyle={{
                borderWidth: 2,
        ...
        <TabView.Item style={{ width: "100%" }}>
          {/* 公告 */}
          <ListItem>
              <ListItem.Title>此课程还没有发布公告!</ListItem.Title>
          </ListItem>
        </TabView.Item>
      </TabView>
    </>
